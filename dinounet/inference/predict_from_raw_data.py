...
from dinounet.inference.data_iterators import PreprocessAdapterFromNpy, preprocessing_iterator_fromfiles, \
    preprocessing_iterator_fromnpy
from dinounet.inference.export_prediction import export_prediction_from_logits, \
    convert_predicted_logits_to_segmentation_with_correct_shape
from dinounet.inference.sliding_window_prediction import compute_gaussian, \
    compute_steps_for_sliding_window
from dinounet.utilities.file_path_utilities import get_output_folder, check_workers_alive_and_busy
...
from dinounet.utilities.label_handling.label_handling import determine_num_input_channels
from dinounet.utilities.plans_handling.plans_handler import PlansManager, ConfigurationManager
from dinounet.utilities.utils import create_lists_from_splitted_dataset_folder
...
class nnUNetPredictor(object):
    ...
    def initialize_from_trained_model_folder(self, model_training_output_dir: str,
        ...
        trainer_class = recursive_find_python_class(join(dinounet.__path__[0], "training", "nnUNetTrainer"),
                                                    trainer_name, 'dinounet.training.nnUNetTrainer')
    ...
    def _internal_maybe_mirror_and_predict_with_target(self, x: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        mirror_axes = self.allowed_mirroring_axes if self.use_mirroring else None
        prediction = self.network(x, target)
        ...
        if mirror_axes is not None:
            ...
            for axes in axes_combinations:
                prediction += torch.flip(self.network(torch.flip(x, (*axes,)), torch.flip(target, (*axes,))), (*axes,))
            prediction /= (len(axes_combinations) + 1)
        return prediction
    ...
    def _internal_predict_sliding_window_return_logits_with_target(self,
                                                       data: torch.Tensor,
                                                       target: torch.Tensor,
                                                       slicers,
                                                       do_on_device: bool = True,
        ...
        try:
            ...
            if self.verbose:
                print(f'move image to device {results_device}')
            data = data.to(results_device)
            target = target.to(results_device)
            ...
            for sl in tqdm(slicers, disable=not self.allow_tqdm):
                ...
                workon_tar = target[sl][None]
                workon_tar = workon_tar.to(self.device, non_blocking=False)

                prediction = self._internal_maybe_mirror_and_predict_with_target(workon, workon_tar)[0].to(results_device)
    ...
    def predict_sliding_window_return_logits_with_target(self, input_image: torch.Tensor, input_mask: torch.Tensor) \
            -> Union[np.ndarray, torch.Tensor]:
        assert isinstance(input_image, torch.Tensor)
        self.network = self.network.to(self.device)
        self.network.eval()
        ...
        with torch.no_grad():
            with torch.autocast(self.device.type, enabled=True) if self.device.type == 'cuda' else dummy_context():
                ...
                if self.perform_everything_on_device and self.device != 'cpu':
                    # we need to try except here because we can run OOM in which case we need to fall back to CPU as a results device
                    try:
                        predicted_logits = self._internal_predict_sliding_window_return_logits_with_target(data, input_mask, slicers,
                                                                                               self.perform_everything_on_device)
                    except RuntimeError:
                        print(
                            'Prediction on device was unsuccessful, probably due to a lack of memory. Moving results arrays to CPU')
                        empty_cache(self.device)
                        predicted_logits = self._internal_predict_sliding_window_return_logits_with_target(data, input_mask,
                                                                                                           slicers, False)
                else:
                    predicted_logits = self._internal_predict_sliding_window_return_logits_with_target(data, input_mask, slicers,
                                                                                           self.perform_everything_on_device)
...
if __name__ == '__main__':
    ...
    from dinounet.paths import nnUNet_results, nnUNet_raw, nnUNet_preprocessed 
    import shutil
    from tqdm import tqdm
    ...
    dataset_id = 'Dataset200_Brats2024'
    skip_prediction = False
    folds = [0]

    for fold in folds:
        split_file = join(nnUNet_preprocessed, dataset_id, 'splits_final.json')
        data = load_json(split_file)
        imageTs_folder = join(nnUNet_raw, dataset_id, 'imagesTs_{}'.format(fold))
        maybe_mkdir_p(imageTs_folder)
        shutil.rmtree(imageTs_folder)
        maybe_mkdir_p(imageTs_folder)
        raw_images_folder = join(nnUNet_raw, dataset_id, 'imagesTr')
        validation_output_folder = join(nnUNet_raw, dataset_id, 'imagesTs_pred_{}'.format(fold))
        # validation_output_folder = join(nnUNet_raw, dataset_id, 'imagesTs_pred_nnunet_{}'.format(fold))

        for da in tqdm(data[fold]['test']):
            for pid in os.listdir(raw_images_folder):
                if da in pid:
                    shutil.copyfile(
                        join(raw_images_folder, pid),
                        join(imageTs_folder, pid)
                    )
        print('all imagesTs:', len(os.listdir(imageTs_folder)))
        ...
        if not skip_prediction:
            ...
            predictor.initialize_from_trained_model_folder(
                join(nnUNet_results, 'Dataset200_Brats2024/CIANetTrainer_2000epochs__nnUNetPlans_CIANet__3d_fullres'),
                # join(nnUNet_results, 'Dataset200_Brats2024/nnUNetTrainer__nnUNetPlans__3d_fullres'),
                use_folds=(fold,),
                checkpoint_name='checkpoint_final.pth',
            )
            predictor.predict_from_files(imageTs_folder,
        ...
        metrics = compute_metrics_on_folder(
            join(nnUNet_preprocessed, dataset_id, 'gt_segmentations'),
            validation_output_folder,
