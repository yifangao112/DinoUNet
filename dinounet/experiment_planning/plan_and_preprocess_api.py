...
from dinounet.experiment_planning.dataset_fingerprint.fingerprint_extractor import DatasetFingerprintExtractor
from dinounet.experiment_planning.experiment_planners.default_experiment_planner import ExperimentPlanner
from dinounet.experiment_planning.verify_dataset_integrity import verify_dataset_integrity
from dinounet.paths import nnUNet_raw, nnUNet_preprocessed
from dinounet.utilities.dataset_name_id_conversion import convert_id_to_dataset_name
...
def extract_fingerprints(dataset_ids: List[int], fingerprint_extractor_class_name: str = 'DatasetFingerprintExtractor',
    ...
    """
    fingerprint_extractor_class = recursive_find_python_class(join(dinounet.__path__[0], "experiment_planning"),
...
def plan_experiment_dataset(dataset_id: int,
                            ...
                            force_target_shape: Optional[Tuple[int, ...]] = None,
                            max_batch_size: int = 32,
                            force_n_stages: Optional[int] = None) -> Tuple[dict, str]:
    """
    ...
    planner = experiment_planner_class(dataset_id,
                                       ...
                                       force_target_shape=force_target_shape,
                                       max_batch_size=max_batch_size,
                                       force_n_stages=force_n_stages,
...
def plan_experiments(dataset_ids: List[int], experiment_planner_class_name: str = 'ExperimentPlanner',
                     ...
                     force_target_shape: Optional[Tuple[int, ...]] = None,
                     max_batch_size: int = 32,
                     force_n_stages: Optional[int] = None):
    """
    overwrite_target_spacing ONLY applies to 3d_fullres and 3d_cascade fullres!
    """
    ...
    for d in dataset_ids:
        _, plans_identifier = plan_experiment_dataset(d, experiment_planner, gpu_memory_target_in_gb,
                                                      ...
                                                      force_target_shape, max_batch_size, force_n_stages)
