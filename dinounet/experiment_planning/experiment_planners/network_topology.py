...
def get_pool_and_conv_props(spacing, patch_size, min_feature_map_size, max_numpool):
    ...
    :return:
    """
    # 如果max_numpool被设置为强制层数-1，我们需要确保达到指定的层数
    force_stages = max_numpool + 1 if max_numpool < 999999 else None
    ...
    while True:
        ...
        if len(valid_axes_for_pool) == 1:
            if current_size[valid_axes_for_pool[0]] >= 3 * min_feature_map_size:
                ...
                break
        if len(valid_axes_for_pool) < 1:
            # 如果强制指定了层数，我们需要继续池化直到达到指定层数
            if force_stages is not None and len(pool_op_kernel_sizes) < force_stages:
                # 强制继续池化，即使可能违反一些约束
                valid_axes_for_pool = [i for i in range(dim) if num_pool_per_axis[i] < max_numpool and current_size[i] >= 2]
                if len(valid_axes_for_pool) == 0:
                    # 如果仍然没有可池化的轴，选择最小的轴进行池化
                    valid_axes_for_pool = [np.argmin(current_size)]
            else:
                break
