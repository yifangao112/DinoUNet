...
import dinounet
from dinounet.imageio.natural_image_reader_writer import NaturalImage2DIO
from dinounet.imageio.nibabel_reader_writer import NibabelIO, NibabelIOWithReorient
from dinounet.imageio.simpleitk_reader_writer import SimpleITKIO
from dinounet.imageio.tif_reader_writer import Tiff3DIO
...
def determine_reader_writer_from_dataset_json(dataset_json_content: dict, example_file: str = None,
    ...
    if 'overwrite_image_reader_writer' in dataset_json_content.keys() and \
        ...
        # trying to find that class in the dinounet.imageio module
        try:
...
def recursive_find_reader_writer_by_name(rw_class_name: str) -> Type[BaseReaderWriter]:
    ret = recursive_find_python_class(join(dinounet.__path__[0], "imageio"), rw_class_name, 'dinounet.imageio')
    if ret is None:
        raise RuntimeError("Unable to find reader writer class '%s'. Please make sure this class is located in the "
                           "dinounet.imageio module." % rw_class_name)
    else:
        return ret
